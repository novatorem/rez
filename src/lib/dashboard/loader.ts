...
export class DashboardDataLoader {
  ...
  async loadFriends(): Promise<Friend[]> {
    ...
    const [{ data: friendUsers }, { data: friendStatuses }] = await Promise.all([
      this.supabase.from('users').select('id, username, display_name').in('id', friendIds),
      this.supabase.from('profiles').select('id, status, updated_at').in('id', friendIds)
    ]);
